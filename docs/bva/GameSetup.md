...
- **TC9: Duplicate names rejected** ( :white_check_mark: )
    - **State of the system**: 3 registrations where two players are named Yuki.
    - **Expected output**: throws `IllegalArgumentException`.

- **TC11: Registration names are trimmed** ( :white_check_mark: )
    - **State of the system**: registration name `"  Yuki  "`.
    - **Expected output**: built player name is `"Yuki"`.
